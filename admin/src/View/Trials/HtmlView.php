...
namespace NPEU\Component\Trials\Administrator\View\Trials;
...
class HtmlView extends BaseHtmlView {
    ...
    */
    ...
    function display($tpl = null) {
        ...
        $this->canDo = ContentHelper::getActions('com_trials');
    ...
    {
        $title = Text::_('COM_TRIALS_MANAGER_RECORDS', 'smiley-2');
        ...
        if ($this->canDo->get('core.create')) {
            ToolBarHelper::addNew('trial.add', 'JTOOLBAR_NEW');
        }
        if ($this->canDo->get('core.edit')) {
            ToolBarHelper::editList('trial.edit', 'JTOOLBAR_EDIT');
        }


        if (!$this->isEmptyState && ($this->canDo->get('core.edit.state'))) {
            if ($this->canDo->get('core.delete')) {
                //ToolBarHelper::deleteList('', 'trials.delete', 'JTOOLBAR_DELETE');
                if ($this->state->get('filter.published') != -2) {
                    ToolBarHelper::deleteList('', 'trials.trash', 'JTOOLBAR_DELETE');
                }
            }

            if ($this->canDo->get('core.edit') || Factory::getApplication()->getIdentity()->authorise('core.manage', 'com_checkin')) {
                ToolBarHelper::checkin('trials.checkin');
            }
        ...
        if (!$this->isEmptyState && $this->state->get('filter.published') == -2 && $this->canDo->get('core.delete')) {
            ToolBarHelper::deleteList('', 'trials.delete', 'JTOOLBAR_EMPTY_TRASH');
            /*$toolbar->delete('trials.delete', 'JTOOLBAR_EMPTY_TRASH')
        ...
        if ($this->canDo->get('core.admin')) {
            ...
            ToolBarHelper::preferences('com_trials');
        }
    }
}