...
namespace NPEU\Component\Trials\Site\View\Trials;
...
class HtmlView extends BaseHtmlView {
    ...
    public function display($template = null)
    {
        ...
        $app->getLanguage()->load('com_trials', JPATH_COMPONENT_ADMINISTRATOR);
        ...
        #echo '<pre>'; var_dump($this->menu_params->get('page_title')); echo '</pre>'; exit;
        ...
        // having to manually redeclare them, along with any other properties of the form that may be
        // useful:
        //$this->setModel($this->getModel('trials'));
        #jimport('joomla.application.component.model');
        #JModelLegacy::addIncludePath(JPATH_SITE . '/components/com_trials/models');
        require JPATH_SITE . '/components/com_trials/models/trial.php';
        $trials_model = JModelLegacy::getInstance('Trialform', 'TrialsModel');
        #echo '<pre>'; var_dump($trials_model); echo '</pre>'; exit;
        $form = $trials_model->getForm();
        ...
        */
        ...
        if ($layout != 'default') {
            ...
            $page_title = Text::_('COM_TRIALS_PAGE_TITLE_' . strtoupper($layout));
        ...
        if ($user_is_root) {
            ...
            $authorised = $user->authorise('core.create', 'com_trials');
        } elseif ($is_own) {
            $authorised = $user->authorise('core.edit.own', 'com_trials');
        }
        else {
            $authorised = $user->authorise('core.edit', 'com_trials');
        }
